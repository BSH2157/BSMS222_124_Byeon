...
my_df <- data.frame(name = state_name, rank = rank)
my_df
```
...
rank <- rank(murders$population)
ind <- order(rank)
state_name <- murders$state[ind]
my_df <- data.frame(name = state_name, rank = rank[ind])
my_df
```
...
city <- c("Beijing", "Lagos", "Paris", "Rio de Janeiro", 
          "San Juan", "Toronto")
city_temps <- data.frame(name = city, temperature = temp, Fahrenheit_temperature = Ftemp)
city_temps
```
...
```{r}
Belowww <- murder_rate < mean(murder_rate)
sum(Belowww)
```
...
```{r}
matabb <- match( c("AK", "MI", "IA"), murders$abb)
murders$state[matabb]
```
...
```{r}
matabb <- which(murders$abb %in% c("MA", "ME", "MI", "MO", "MU"))
murders$abb[matabb]
```
...
```{r}
incorrectabb <- c("MA", "ME", "MI", "MO", "MU")
ind <- which(!incorrectabb%in%murders$abb)
incorrectabb[ind]
```
...
```{r}
makehist <- with(murders, population/10^6)
