...
```{r}
d_luad %>% count(shortLetterCode) #질문거리
```
...
```
'A1: This is the code that use 0 instead of NA in shortlettercode column'
'A2: This is the code that seperate plot between lusc, luad, with type column.'
...
'A:Histogram's y axis is count, but density plot's y axis is density. So, the plot only show the histogram'
...
'It doesn't follow nomal distribution, and it startificated. So, we need to visualize it more precisely, different manners.'
...
Can you tell the difference between boxplot and density plot?

'In contrast with density plot, boxplot just show summarised simple imformation of median, and quantile of 25%~75% and outlier.'

We can try a violin plot.
...
'In contrast boxplot, violin plot show more imformation about distribution like density plot. But it can mesure absolute value, and not show median, outlier.'
...
head(d_luad)
# assign p for plot
p <- d_luad %>% mutate(Age=age_at_diagnosis/365) %>%
  ggplot(aes(Age, fill=tumor_stage, alpha=0.5)) +
  labs(x ='Diagnnosied Age(years)', y = 'LUAD patients density',
       title = 'TCGA: Lung cancer adenocarcinoma distribution by gender') +
